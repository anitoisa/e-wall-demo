...
revisionPanel=function(id){const m=state.mode,s=state.segment;
 if(id==='main')return `<article class="panel clean-main full-main" data-id="main"><div class="image-only" data-key="main-image"><img src="${root}assets/ue/${m}-${String(s+1).padStart(2,'0')}.webp" alt="UE 預錄預覽：${MAIN_COPY[m][s]}"></div><span class="ue-recorded-badge">UE 預錄預覽</span><div class="main-info" data-key="main-overlay"><div class="main-copy"><span>0${MODES.findIndex(x=>x.id===m)} · ${{idle:'序章',life:'建築生命履歷',bim:'數位孿生定位',data:'建築 BOS'}[m]} <i>${s+1}/${CHAPTERS[m].segmentCount}</i></span><strong>${MAIN_COPY[m][s]}</strong></div><img data-key="main-logo" class="corner-logo" src="${root}assets/logo.png" alt="ANLB INSIDE"></div></article>`;
 if(m==='idle'){
