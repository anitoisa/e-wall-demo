...
document.documentElement.dataset.mode=m.id;document.documentElement.dataset.standby=String(!!state.standby);document.documentElement.style.setProperty('--accent',state.standby?'#3FB4F0':m.color);document.body.classList.toggle('paused',!state.playing);document.documentElement.style.setProperty('--phase',`-${state.elapsed}s`);
 ...
 refreshScores();modelSync();UESequence.render(state);
 resize();
}
function connection(ok){online=ok;document.body.classList.toggle('offline',!ok);const el=document.getElementById('connection');if(el)el.textContent=UESequence.error?UESequence.error:UESequence.loading?'● 正在準備 UE 連續畫面':frozen?'固定分鏡預覽':ok?(state.standby?'● 待機 · 全牆預覽':'● 演示模式 · UE 實機擷取'):'● 準備中';document.querySelectorAll('[data-action],[data-mode]').forEach(b=>b.disabled=!ok||frozen);}
...
let commandToken=0;
async function command(action,mode){
 if(frozen)return;
 const token=++commandToken;
 const starts=action==='start'||(action==='toggle'&&!state.playing)||(action==='mode'&&state.standby);
 if(starts){try{await UESequence.prepare();}catch{connection(false);return;}if(token!==commandToken)return;}
 state=DemoClock.command(action,mode);render();connection(true);
}
