...
function connection(ok){online=ok;document.body.classList.toggle('offline',!ok);const el=document.getElementById('connection');if(el)el.textContent=UESequence.error?UESequence.error:UESequence.loading?'● 正在準備 UE 連續畫面':frozen?'固定分鏡預覽':ok?(state.standby?'● 待機 · 全牆預覽':'● 演示模式 · UE 預錄'):'● 準備中';document.querySelectorAll('[data-action],[data-mode]').forEach(b=>b.disabled=!ok||frozen);}
