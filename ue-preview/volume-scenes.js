...
export function buildVolume(s,h){
 ...
 if(kind==='idle-s1'){
  ...
  add((v,t)=>{const a=Math.min(v,1),b=Math.max(0,v-1);service.position.z=b*5;service.position.y=-b*1.4;service.traverse(o=>{if(o.isMesh){o.material.transparent=true;o.material.opacity=.35+.65*a;}});flow.forEach((o,i)=>{o.visible=v>.8;o.position.x=-9+(i*2.8+t*.8)%16;});group.rotation.y=-.35;group.rotation.x=.1;});return true;
 }
 if(kind==='life-s1'){
 ...
 if(kind==='bim-a'){
  ...
  add(v=>{const a=Math.min(v,1),b=Math.max(0,v-1);space.position.x=-7*a-2*b;space.scale.z=.015+.985*a;systems.position.x=1.5*a;systems.visible=v>.02;systems.scale.setScalar(.15+a*.85);history.position.x=11;history.visible=b>.02;history.scale.setScalar(.1+b*.9);group.rotation.y=-.17;});return true;
 }
 if(kind==='bim-d'){
  ...
  add((v,t)=>{pump.rotation.y=-.12;system.visible=v>.05;system.scale.setScalar(.9+Math.min(v,1)*.1);dots.forEach((d,i)=>{d.position.x=5+(i*1.5+t*.6)%6;});group.rotation.y=-.12;});return true;
 }
 if(kind==='bim-s1'){
  ...
  add(v=>{const a=Math.min(v,1),b=Math.max(0,v-1);floors.forEach((g,i)=>{g.scale.z=.015+.985*a;g.position.y=(i-5.5)*1.3;g.position.x=i===5?b*5:0;g.position.z=i===5?b*3:0;});group.rotation.y=-.12;});return true;
 }
 if(kind==='bim-s4'){
  ...
  add((v,t)=>{const a=Math.min(v,1);covers.forEach((o,i)=>{o.position.x=(i?1:-1)*(6.35+a*7);o.material.transparent=true;o.material.opacity=1-a*.94;o.visible=a<.99;});flow.forEach((o,i)=>{o.visible=v>1.05;o.position.x=-10.5+(i*3+t*1.5)%21;});marker.visible=v>1.05;group.rotation.y=-.1;});return true;
 }
 return false;
}
