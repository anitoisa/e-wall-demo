(async()=>{
 ...
 const demoStyle=document.createElement('link');demoStyle.rel='stylesheet';demoStyle.href=base+'demo.css?v=sequence2';document.head.appendChild(demoStyle);
 ...
 try{const response=await fetch(base+'chapters.json',{cache:'no-store'});if(!response.ok)throw Error('章節設定無法載入');globalThis.CHAPTERS=await response.json();
  ...
  if(document.body.dataset.view==='ipad'){const tabletStyle=document.createElement('link');tabletStyle.rel='stylesheet';tabletStyle.href=base+'ipad-preview.css';document.head.appendChild(tabletStyle);await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+'ipad-preview.js?v=sequence2';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});}
  for(const file of ['scoring.js','revision.js','showcase-ui.js','brand-exhibition.js','demo-clock.js','ue-sequence.js','app.js'])await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+file+'?v=sequence2';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});
