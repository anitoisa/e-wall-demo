(async()=>{
 ...
 try{const response=await fetch(base+'chapters.json',{cache:'no-store'});if(!response.ok)throw Error('章節設定無法載入');globalThis.CHAPTERS=await response.json();
  ...
  for(const file of ['scoring.js','revision.js','showcase-ui.js','brand-exhibition.js','demo-clock.js','ue-sequence.js','app.js'])await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+file+'?v=scope-colors-20260910';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});
