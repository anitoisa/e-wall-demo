(async()=>{
 ...
 const demoStyle=document.createElement('link');demoStyle.rel='stylesheet';demoStyle.href=base+'demo.css?v=scope-colors-20260910';document.head.appendChild(demoStyle);
 ...
 try{const response=await fetch(base+'chapters.json',{cache:'no-store'});if(!response.ok)throw Error('章節設定無法載入');globalThis.CHAPTERS=await response.json();
  ...
  if(document.body.dataset.view==='ipad'){const tabletStyle=document.createElement('link');tabletStyle.rel='stylesheet';tabletStyle.href=base+'ipad-preview.css';document.head.appendChild(tabletStyle);await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+'ipad-preview.js?v=scope-colors-20260910';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});}
  for(const css of ['source-review.css','review-motion.css']){const el=document.createElement('link');el.rel='stylesheet';el.href=base+css+'?v=content-motion-20260911';document.head.appendChild(el);}
  for(const file of ['scoring.js','revision.js','showcase-ui.js','brand-exhibition.js','source-review.js','review-motion.js','demo-clock.js','ue-sequence.js','app.js'])await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+file+'?v=content-motion-20260911';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});
  ...
  import(new URL(base+'showcase.js?v=content-motion-20260911',location.href).href).catch(error=>{document.querySelectorAll('.show-loading').forEach(el=>el.textContent='立體圖像無法載入，請重新整理');console.error(error);});
