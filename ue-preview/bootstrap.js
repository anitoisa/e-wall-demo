(async()=>{
 ...
 const demoStyle=document.createElement('link');demoStyle.rel='stylesheet';demoStyle.href=base+'demo.css?v=standby2';document.head.appendChild(demoStyle);
 ...
 try{const response=await fetch(base+'chapters.json',{cache:'no-store'});if(!response.ok)throw Error('章節設定無法載入');globalThis.CHAPTERS=await response.json();
  ...
  for(const file of ['scoring.js','revision.js','showcase-ui.js','brand-exhibition.js','demo-clock.js','app.js'])await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+file+'?v=standby2';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});
