...
function updateScene(s,state,t,now,reduced){
 ...
 const isBuilding=s.kind==='data-a';if(isBuilding){s.camera.position.set(0,36*p,150);s.camera.lookAt(0,6*p,0);}else{const a=state.mode==='data'?0:Math.min(s.stage,1);s.camera.position.set(a*7,a*6,40);s.camera.lookAt(0,0,0);if(state.segment===0)s.group.scale.setScalar(.72+.28*s.reveal);else s.group.scale.setScalar(1);}
 s.lastSnapshot=snap;
}
function draw(now){
