...
function panel(id){if(id==='d')return vacantD();if(state.standby!==false&&!frozen)return `<article class="panel standby-panel ${portrait(id)?'portrait':''}" data-id="${id}"><img src="${root}assets/logo.png" alt="ANLB INSIDE"><strong>第五代住宅宣言</strong><span>AI NATIVE LIVING BUILDING</span></article>`;const m=current(),seg=state.segment;let body='',cl='',note='概念圖解 · 非即時建物資料';
...
function render(){const m=current();const waiting=!frozen&&state.standby!==false;document.documentElement.dataset.mode=m.id;document.documentElement.dataset.standby=String(waiting);document.documentElement.style.setProperty('--accent',waiting?'#3FB4F0':m.color);document.body.classList.toggle('paused',!state.playing);document.documentElement.style.setProperty('--phase',`-${state.elapsed}s`);
 if(view==='wall'){document.body.classList.remove('is-standby');document.getElementById('wall-standby').hidden=true;for(const el of document.querySelectorAll('#wall,#modes,.wall-heading,.transport'))el.inert=false;if(waiting&&document.getElementById('inspect').open)document.getElementById('inspect').close();}
 ...
 const key=String(waiting)+':'+m.id+state.segment+':'+state.scenarioSeed;if(key!==lastKey){
...
function connection(ok){online=ok;document.body.classList.toggle('offline',!ok);const el=document.getElementById('connection');if(el)el.textContent=frozen?'固定分鏡預覽':ok?'● 獨立線上預覽 · 不連接展場':'● 準備中';document.querySelectorAll('[data-action],[data-mode]').forEach(b=>b.disabled=!ok||frozen);}
async function poll(){if(frozen){render();connection(false);return}state=ANLBPreviewClock.snapshot();render();connection(true);setTimeout(poll,250);}
let pendingCommands=[];
async function command(action,mode){if(!online||frozen)return;state=ANLBPreviewClock.command(action,mode);render();connection(true);}
