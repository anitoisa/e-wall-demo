(async()=>{
 ...
 const liveStyle=document.createElement('link');liveStyle.rel='stylesheet';liveStyle.href=base+'live-standby.css?v=20260908';document.head.appendChild(liveStyle);
 ...
 try{const response=await fetch(base+'chapters.json',{cache:'no-store'});if(!response.ok)throw Error('章節設定無法載入');globalThis.CHAPTERS=await response.json();
  ...
  const vacancyStyle=document.createElement('link');vacancyStyle.rel='stylesheet';vacancyStyle.href=base+'d-vacancy.css';document.head.appendChild(vacancyStyle);
  ...
  for(const file of ['preview-clock.js','scoring.js','revision.js','showcase-ui.js','brand-exhibition.js','source-review.js','review-motion.js','d-vacancy.js','motion-upgrade.js','app.js'])await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=base+file+'?v=motion-upgrade-0912';s.onload=resolve;s.onerror=reject;document.body.appendChild(s);});
