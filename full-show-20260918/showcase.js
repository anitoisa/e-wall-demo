...
import {buildVolume} from './volume-scenes.js?v=selection-20260915';
import {holdVolume} from './hold-volume.js?v=selection-20260915';
...
function makeScene(kind){
 ...
 if(kind==='data-a'){
  ...
  add((v,t,snap,p)=>{if(!s.ready)return;const map=new Map(snap.records.map(r=>[r.id,r]));for(const u of s.units){u.mesh.material.color.set(map.get(u.id).color);if(Math.abs(p-s.geometryProgress)>.0002){const positions=u.mesh.geometry.attributes.position;for(let j=0;j<u.flat.length;j++)positions.array[j]=mix(u.flat[j],u.final[j],p);positions.needsUpdate=true;u.mesh.geometry.computeVertexNormals();}u.mesh.userData.score=map.get(u.id).score;}s.geometryProgress=p;s.structures.forEach(o=>{o.material.opacity=p*.8;o.visible=p>.02;});s.ground.visible=p>.1;s.ground.material.opacity=p*.75;group.rotation.y=p*[.58,-.6,2.5,-2.5][snap.index];
   selections.forEach((edge,i)=>{const u=s.units.find(u=>u.id===snap.selected[i].id);u.mesh.geometry.computeBoundingBox();u.mesh.geometry.boundingBox.getCenter(edge.position);u.mesh.geometry.boundingBox.getSize(edge.scale);edge.scale.addScalar(.12);edge.visible=t>=7;edge.userData.reveal=ease((t-7)/.8);edge.material.opacity=edge.userData.reveal*ExSelection.value(t,7.8,{reduced:s.reduced,manual:s.manualReveal});});
...
function updateScene(s,state,t,now,reduced){
 ...
 const motion=globalThis.ExMotion?.sample(s.kind==='bim-d'?'s3':s.kind.split('-')[1],now);
 const blend=reduced?1:ease((now-s.change)/4200);s.stage=motion?motion.v:mix(s.fromStage||0,s.targetStage,blend);
  if(motion)t=motion.t;
 s.manualReveal=!!(motion?.manual&&!state.playing);
 s.reduced=reduced;
 s.reveal=motion?.p??1;
...
function draw(now){
 ...
 for(const el of document.querySelectorAll('.show-viewport')){
  ...
  updateScene(s,state,t,now,reduced);holdVolume(s,state,now);el.dataset.ready=s.ready?'true':'false';if(!s.ready)continue;active++;
...
globalThis.showcaseDiagnostics=()=>({rendererCount:1,modelLoads,rendererFrames,scenes:[...scenes.values()].map(s=>({kind:s.kind,ready:s.ready,error:s.error,stage:s.stage,pose:s.pose,sequence:s.sequence,attention:[...(s.attentionMaterials||new Map()).entries()].map(([o,e])=>({name:o.name,type:o.type,emissive:e.material.emissiveIntensity,opacity:e.material.opacity,color:e.material.color.getHexString(),position:o.position.toArray(),scale:o.scale.toArray()})),covers:s.holdCovers?.map(o=>({opacity:o.material.opacity,visible:o.visible,x:o.position.x})),extraEdges:(()=>{let count=0;s.group.traverse(o=>{if(o.name==='hold-decoration')count++;});return count;})(),seed:s.seed,segment:s.segment,selectionOpacity:s.auxiliary?.selections.map(o=>o.material.opacity),selected:s.lastSnapshot?.selected.map(r=>r.id),histogram:s.lastSnapshot?.histogram,units:s.units?.map(u=>({id:u.id,color:'#'+u.mesh.material.color.getHexString(),score:u.mesh.userData.score}))}))});
