...
function vacantD(){return `<article class="panel d-narration-panel" data-id="d" aria-label="D 字幕與語音球・本機暫用預覽"><iframe class="d-narration-view" data-key="d-narration-v12" title="D 字幕與語音球（暫用字幕、無聲示意）" src="${root}d-narration/index.html?v=audio-20260915" loading="eager"></iframe></article>`;}
...
document.title='ANLB｜D 語音球與字幕・本機整合預覽';
function vacancyMotion(now){
 ...
 if(st.mode==='idle'&&st.segment===2)document.querySelectorAll('.building-promises').forEach(root=>{
  const frame=root.querySelector('.model-view'),anchor=frame?.contentWindow?.modelSnapshot?.().anchor,svg=root.querySelector('.model-information-link');
  if(anchor&&svg){const x=anchor.x*frame.clientWidth/root.clientWidth*1740,y=anchor.y*frame.clientHeight/svg.clientHeight*650;svg.querySelector('path').setAttribute('d',`M${x} ${y}H1320Q1360 ${y} 1360 ${y+40}V530H1390`);svg.querySelector('circle').setAttribute('cx',x);svg.querySelector('circle').setAttribute('cy',y);}
  const info=root.querySelector('.model-record-link');if(info)info.style.opacity=String(reduced?1:Math.max(0,Math.min(1,(t-3.6)/.7)));
 });
 if(st.mode==='data'){
