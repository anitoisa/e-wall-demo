...
function icon(type){const paths={
 ...
 record:'<path d="M29 13h48l19 19v75H29zM77 13v22h19"/><path class="record-lines" d="M43 49h37M43 65h37M43 81h24"/><circle class="pulse-dot" cx="88" cy="91" r="15"/><path d="m81 91 5 5 9-11"/>',
 pipe:'<path d="M18 24h45v38h37M18 97h45V62M100 19v78"/><circle cx="63" cy="62" r="12"/><path class="flow-path" d="M18 24h45v38h37"/>',
 layers:'<path d="m13 39 47-25 47 25-47 25Z"/><path class="draw" d="M13 62l47 25 47-25M13 85l47 25 47-25"/>',
 target:'<circle cx="60" cy="60" r="33"/><circle class="pulse-dot" cx="60" cy="60" r="13"/><path d="M60 6v30M60 84v30M6 60h30M84 60h30"/>',
 pulse:'<path class="draw" d="M10 64h24l13-34 22 64 14-30h27"/><circle cx="60" cy="60" r="51"/>',
 handover:'<path d="M20 39h78L83 24M100 81H22l15 15M22 17v86M98 17v86"/><path class="draw" d="m42 59 12 12 25-25"/>',
 structure:'<path d="M22 108V13h76v95M22 42h76M22 75h76"/><path class="draw" d="M22 13l76 62M98 13 22 75M22 42l76 66M98 42l-76 66"/>',
 ...
 return `<svg class="icon" data-icon-kind="${type}" viewBox="0 0 120 120" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">${paths[type]||paths.record}</svg>`;
}
...
function resize(){
 ...
 const mainSlot=document.querySelector('#wall .slot[data-id="main"]'),logoWidth=mainSlot?mainSlot.clientWidth*320/1920:null;
 document.querySelectorAll('.slot').forEach(slot=>{const p=slot.querySelector('.panel'),scale=slot.clientWidth/(portrait(slot.dataset.id)?1080:1920);p.style.transform=`scale(${scale})`;if(logoWidth&&p.classList.contains('standby-logo-only'))p.style.setProperty('--standby-logo-width',`${logoWidth/scale}px`);});
...
function morph(oldNode,newNode){
 ...
 for(const attr of [...newNode.attributes]){if(iframe&&['src','class'].includes(attr.name))continue;const value=attr.name==='class'?(attr.value+' '+[...oldNode.classList].filter(c=>c.startsWith('attention-')).join(' ')).trim():attr.value;if(oldNode.getAttribute(attr.name)!==value)oldNode.setAttribute(attr.name,value);}
 if(iframe)return;
...
function modelSync(){globalThis.showcaseState={...state,elapsed:revisionElapsed(),received:performance.now()};document.querySelectorAll('.model-view').forEach(frame=>frame.contentWindow?.postMessage({type:'anlb-scene',mode:state.mode,segment:state.segment,playing:state.playing,elapsed:frame.dataset.role==='exhibit'&&globalThis.ExMotion?ExMotion.sample('c').t:revisionElapsed(frame.dataset.role)},location.origin));}
...
if(frozen)document.documentElement.style.transition='none';
...
let transitionKey='',uiTransition=null;
function render(){
...
function renderContent(){const m=current();const waiting=!frozen&&state.standby!==false;document.documentElement.dataset.mode=m.id;document.documentElement.dataset.standby=String(waiting);document.documentElement.style.setProperty('--accent',waiting?'#3FB4F0':m.color);document.body.classList.toggle('paused',!state.playing);document.documentElement.style.setProperty('--phase',`-${state.elapsed}s`);
 if(view==='wall'){document.body.classList.remove('is-standby');document.getElementById('wall-standby').hidden=true;for(const el of document.querySelectorAll('#wall,#modes,.wall-heading,.transport'))el.inert=false;}
 ...
 const key=String(waiting)+':'+m.id+state.segment+':'+state.scenarioSeed+':'+(state.sceneKey||'');if(key!==lastKey){
 ...
 if(waiting&&!single){document.getElementById('chapter-label').textContent='ANLB / 待機';document.getElementById('chapter-claim').textContent='第五代住宅宣言';document.getElementById('segment-label').textContent='全牆待機預覽';document.getElementById('story-note').textContent='按開始或選擇情境，開始展演';document.getElementById('play').textContent='▶ 開始';document.querySelectorAll('button[data-mode]').forEach(b=>b.setAttribute('aria-pressed','false'));}
...
if(view==='wall'){const stage=document.createElement('div');stage.className='wall-stage';const wall=document.getElementById('wall'),standby=document.getElementById('wall-standby');standby.before(stage);stage.append(standby,wall);const dock=document.createElement('aside');dock.id='ipad-dock';dock.setAttribute('aria-label','iPad 同步預覽');const url=new URL('ipad.html',location.href);url.searchParams.set('embedded','1');if(globalThis.ExIntro?.enabled)url.searchParams.set('narration',ExIntro.full?'full':'intro');if(frozen){for(const [key,value] of query)url.searchParams.set(key,value);}dock.innerHTML=`<iframe title="iPad 同步控制預覽" src="${url.pathname+url.search}" loading="eager"></iframe><a href="ipad.html" target="_blank" rel="noopener">iPad 同步預覽 ↗</a>`;stage.append(dock);}
if(view==='wall'){const controls=document.querySelector('.transport');const note=document.getElementById('story-note');document.getElementById('modes').after(note);document.querySelector('main').prepend(controls);}
